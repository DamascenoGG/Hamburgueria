...
export default function OrganogramaManager() {
  ...
  const handleSaveEdit = (): void => {
    if (!editingFuncionario) return;
    if (
      !editingFuncionario.nome ||
      !editingFuncionario.login ||
      !editingFuncionario.senha
    ) {
    ...
    setEquipe((prev) =>
      prev.map((f) =>
        f.id === editingFuncionario.id ? editingFuncionario : f,
      ),
    );
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        ...
        {!isAddingNew && !editingFuncionario && (
          <button
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1a1a] p-8 rounded-3xl border border-[#332a1a] shadow-lg max-w-2xl w-full animate-in zoom-in-95 duration-300">
            <h2 className="text-xl font-bold mb-6 text-white flex items-center gap-2">
              ...
              Editando: {editingFuncionario.nome}
            </h2>
            ...
            <div className="space-y-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  type="text"
                  name="login"
                  value={editingFuncionario.login}
                  onChange={handleEditingChange}
                  className="bg-[#0f0f0f] border border-[#332a1a] text-white p-3 rounded-xl focus:ring-2 focus:ring-[#f1a128] outline-none"
                />
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  type="password"
                  name="senha"
                  value={editingFuncionario.senha}
                  onChange={handleEditingChange}
                  className="bg-[#0f0f0f] border border-[#332a1a] text-white p-3 rounded-xl focus:ring-2 focus:ring-[#f1a128] outline-none"
                />
              </div>
                ...
                >
                  ...
                  <td className="p-4 text-right space-x-3">
                    <button
                      ...
                      className="text-[#666] hover:text-[#f1a128] transition-colors"
                    >
                      Editar
                    </button>
                    <button
